...
def map_to_viruses(args, params, filenames):
    ...
    try:
        ...
        if args.bwa is True:
            ...
            cmd='hisat2 --mp %s -t -x %s -p %d -U %s --no-spliced-alignment --all --secondary | samtools view -Sbh -o %s -' % (params.hisat2_mismatch_penalties, args.vrefindex, thread_n, filenames.unmapped_merged_1, filenames.mapped_unsorted_bam)
        else:
            cmd='hisat2 --mp %s -t -x %s -p %d -1 %s -2 %s --no-spliced-alignment --all --secondary | samtools view -Sbh -o %s -' % (params.hisat2_mismatch_penalties, args.vrefindex, thread_n, filenames.unmapped_merged_1, filenames.unmapped_merged_2, filenames.mapped_unsorted_bam)
